# Authors and Citation

## Authors

- **Réginal Exavier**. Author, maintainer.
  [](https://orcid.org/0000-0002-5237-523X)

- **Fernando Shinji Kawakubo**. Author.
  [](https://orcid.org/0000-0002-2045-6318)

- **Peter Zeilhofer**. Author. [](https://orcid.org/0000-0002-7579-6408)

## Citation

Source:
[`DESCRIPTION`](https://github.com/reginalexavier/wcswatin/blob/v0.1.1/DESCRIPTION)

Exavier R, Kawakubo F, Zeilhofer P (2026). *wcswatin: Weather and
Climate Inputs for 'SWAT'*. R package version 0.1.1,
<https://github.com/reginalexavier/wcswatin>.

    @Manual{,
      title = {wcswatin: Weather and Climate Inputs for 'SWAT'},
      author = {Réginal Exavier and Fernando Shinji Kawakubo and Peter Zeilhofer},
      year = {2026},
      note = {R package version 0.1.1},
      url = {https://github.com/reginalexavier/wcswatin},
    }
